...
with st.sidebar:
    ...
    logo_path = Path(r"A:\PROJECTS\MTF_TRADING_SYSTEM\Images\logo.png") 
                    ...
                    with st.spinner("🤖 Training models & generating forecast..."):
                        try:
                            ...
                            if result and result.get('forecast'):
                                ...
                                # Display forecast results in 3 columns with type-safe conversion
                                fcol1, fcol2, fcol3 = st.columns(3)
                                
                                with fcol1:
                                    # Convert to native Python float to avoid numpy format issues
                                    pred_return = float(forecast['predicted_return_pct'])
                                    st.metric(
                                        "Predicted 30-Day Return",
                                        f"{pred_return:.2f}%",
                                        delta=f"{pred_return:.2f}%"
                                    )
                                
                                with fcol2:
                                    current_price = float(result['history']['Close'].iloc[-1])
                                    st.metric(
                                        "Current Price",
                                        f"₹{current_price:.2f}"
                                    )
                                ...
                                with fcol3:
                                    pred_price = float(forecast['final_price'])
                                ...
                                try:
                                    ...
                                    st.warning(f"Unable to display forecast chart: {str(chart_error)}")
                                ...
                                try:
                                    ...
                                    st.warning(f"Unable to display feature importance: {str(e)}")
                                ...
                                with st.expander("📊 Model Performance Metrics", expanded=True):
                                    ...
                                    with mcol3:
                                        dir_acc = float(metrics['direction_accuracy']) * 100
                                        st.metric(
                                            "Direction Accuracy", 
                                            f"{dir_acc:.1f}%"
                                        )
                                        st.caption("Correct Trend Predictions")
                                    
                                    # Model composition info
                                    models_used = []
                                    if 'linear' in agent.models and agent.models.get('linear'):
                                        models_used.append("Linear Regression")
                                    if 'xgb' in agent.models and agent.models.get('xgb'):
                                        models_used.append("XGBoost")
                                    if 'lstm' in agent.models and agent.models.get('lstm'):
                                        models_used.append("LSTM")
                                    
                                    if models_used:
                                        st.info(
                                            f"**Ensemble Models:** {', '.join(models_used)}\n\n"
                                            "Trained on 5 years of historical data with 50+ engineered features."
                                        )
                                    else:
                                ...
                                if result.get('shap_values') is not None:
                                    with st.expander("🎯 Top Feature Importance (SHAP)", expanded=False):
                                        try:
                                            ...
                                            top_values = [float(mean_shap[i]) for i in top_indices]
                                            
                                            importance_df = pd.DataFrame({
                                                'Feature': top_features,
                                                'Importance Score': top_values
                                            })
                                            ...
                                            st.caption(
                                                "SHAP values show which features impact the predictions most."
                                            )
                                        except Exception as shap_error:
                                            st.warning(f"Unable to display feature importance: {str(shap_error)}")
                                
                                # Forecast data table
                                with st.expander("📋 Detailed Forecast Data", expanded=False):
                                    try:
                                        # Convert all arrays to lists of native Python floats
                                        forecast_df = pd.DataFrame({
                                            'Date': forecast['dates'],
                                            'Predicted Price': [float(x) for x in forecast['mean_path']],
                                            'Upper Bound (95%)': [float(x) for x in forecast['upper_band']],
                                            'Lower Bound (95%)': [float(x) for x in forecast['lower_band']]
                                        })
                                        ...
                                        st.warning(f"Unable to display forecast table: {str(table_error)}")
                            ...
                            st.info(
                                ...
                                "```bash\n"
                                "pip install xgboost tensorflow scikit-learn shap\n"
                                "```"
                            )
                            ...
                            st.info(
                                "💡 **Troubleshooting:**\n"
                                "- Ensure ticker has 5+ years of data\n"
